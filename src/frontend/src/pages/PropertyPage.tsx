...
export default function PropertyPage({ propertyId }: PropertyPageProps) {
  ...
  const bookNowLabel = t("property.sticky.booknow") || "Book Now";
  ...
  return (
    <div className="flex flex-col pb-20 md:pb-0">
        ...
        <div className="container">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-16 lg:gap-20">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {property.amenities.map((amenity) => {
                    ...
                    return (
                      <div
                        key={amenity.text}
                        className="flex items-start gap-4 p-6 rounded-lg bg-muted/30 hover:bg-muted/50 transition-all duration-500"
                      >
                    ...
                    <CardContent className="pt-8">
                      <div className="flex items-center gap-4 mb-4">
                        ...
                        <h3 className="text-2xl font-semibold">
                          {t("property.sidebar.capacity")}
                        </h3>
                      </div>
                      <p className="text-muted-foreground text-lg">
                        {t("property.occupancy.capacity.value").replace(
                          "{count}",
                  ...
                  <Card className="hover:shadow-elegant-lg transition-all duration-500 bg-white">
                    <CardContent className="pt-8">
                      <div className="flex items-center gap-4 mb-4">
                        ...
                        <h3 className="text-2xl font-semibold">
                          {t("property.sidebar.minstay")}
                        </h3>
                      </div>
                      <p className="text-muted-foreground text-lg">
                        {t("property.occupancy.minstay.value").replace(
                          "{count}",
                          property.minStay.toString(),
                        )}
                      </p>
                    </CardContent>
                  </Card>
                ...
                <div className="mt-8 p-8 bg-muted/30 rounded-lg">
                  <p className="text-muted-foreground leading-relaxed text-lg font-light">
                    <strong className="text-foreground font-semibold">
                      {t("property.occupancy.note.label")}
                    </strong>{" "}
                    {t("property.occupancy.note")}
                  </p>
                </div>
              </div>
              ...
              <div className="animate-fade-in-up delay-400">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                  {property.images.map((image, index) => (
                    <div
                      key={image}
                      className="relative aspect-[4/3] overflow-hidden rounded-lg group cursor-pointer hover:shadow-elegant-lg transition-all duration-700 bg-muted/20"
                    >
                      <picture>
                        <source
                        ...
                        <img
                          ...
                          decoding="async"
                          fetchPriority="low"
                          width={index < 2 ? 1200 : 1000}
                          height={index < 2 ? 800 : 667}
                        />
                      </picture>
                    </div>
                  ))}
            ...
            <div className="space-y-10">
              {/* Pricing Card — anchor for "Book Now" scroll target */}
              <Card
                ref={bookingRef}
                className="sticky top-32 shadow-elegant-lg hover:shadow-elegant-xl transition-all duration-500 bg-white"
              >
                <CardContent className="pt-8 space-y-8">
                  <div>
                    <Badge
                      variant="secondary"
                      className="text-base px-6 py-3 mb-6"
                    >
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between text-lg">
                      ...
                      <span className="font-semibold">
                        {t("property.sidebar.minstay.nights").replace(
                          "{count}",
                          property.minStay.toString(),
                        )}
                      </span>
                    ...
                    <div className="flex items-center justify-between text-lg">
                      <span className="text-muted-foreground">
                        {t("property.sidebar.checkin")}
                      </span>
                      <span className="font-semibold">
                        {t("property.sidebar.checkin.time")}
                      </span>
                    </div>
                    <div className="flex items-center justify-between text-lg">
                      <span className="text-muted-foreground">
                        {t("property.sidebar.checkout")}
                      </span>
                      <span className="font-semibold">
                        {t("property.sidebar.checkout.time")}
                      </span>
                    </div>
                  </div>
              ...
              <Card className="shadow-elegant hover:shadow-elegant-lg transition-all duration-500 bg-white">
                <CardContent className="pt-8">
                  ...
                  <div className="space-y-4 text-base text-muted-foreground font-light">
                    ...
                    <p>
                      <strong className="text-foreground font-semibold">
                        {t("property.sidebar.contact.email")}
                      </strong>
                      <br />
      ...
      {/* Sticky Book Now Bar — visible on mobile, hidden on lg (sidebar handles desktop) */}
      <div
        className="fixed bottom-0 left-0 right-0 z-50 lg:hidden"
        style={{ boxShadow: "0 -2px 16px 0 rgba(15,61,46,0.10)" }}
        data-ocid="sticky-book-now-bar"
      >
        <div className="bg-white border-t border-border/60 px-4 py-3 flex items-center gap-3">
          ...
          <Button
            ...
            className="shrink-0 font-semibold px-6 py-2.5 text-sm bg-forest-green text-cream hover:bg-forest-green/90 focus-visible:ring-2 focus-visible:ring-forest-green focus-visible:ring-offset-2 transition-colors duration-200"
            data-ocid="sticky-book-now-btn"
          >
            {bookNowLabel}
