...
const PROPERTY_RATINGS: Record<string, number> = {
  schwarzwaldblick: 4.8,
  forsthaus: 4.8,
  fichtenberg: 4.7,
  waldhaus: 4.9,
};
...
export default function HomePage() {
  ...
  const storyText =
    t("home.story.text") ||
    "Nestled between ancient firs and whispering brooks, the Black Forest has been a place of refuge for centuries. Here, where fog drifts gently through cathedral-like tree canopies and the air carries the scent of pine and moss, we have created four extraordinary residences. Each house is a quiet sanctuary — where traditional craftsmanship meets understated luxury, and the forest becomes your companion. We invite you to slow down, to listen to the silence, and to rediscover what truly matters.";
  ...
  return (
    <div className="flex flex-col">
      ...
      {/* Our Story Section */}
      <section className="py-24 md:py-32 lg:py-40 bg-muted/10">
        <div className="container">
          <div className="max-w-4xl mx-auto">
            {/* Thin top divider */}
            <div
              className="w-24 h-px bg-border mx-auto mb-16 md:mb-20"
              aria-hidden="true"
            />
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-serif font-semibold text-center mb-12 md:mb-16 tracking-tight animate-fade-in-up">
              {t("home.story.title") || "Our Story"}
            </h2>
            <div className="text-lg md:text-xl text-muted-foreground leading-relaxed font-light text-center animate-fade-in-up delay-200">
              {storyText}
            </div>
            {/* Thin bottom divider */}
            <div
              className="w-24 h-px bg-border mx-auto mt-16 md:mt-20"
              aria-hidden="true"
            />
          </div>
        </div>
        ...
        <div className="container">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-16">
            {properties.map((property, index) => (
              ...
              >
                <div className="relative h-80 md:h-96 overflow-hidden bg-muted/20">
                  <picture>
                    <source
                      srcSet={property.image.replace(".jpg", ".webp")}
                      type="image/webp"
                    />
                    <img
                      src={property.image}
                ...
                <CardHeader className="space-y-3 md:space-y-4 pt-8 md:pt-10 pb-4 md:pb-6">
                  ...
                  <div className="flex items-center gap-1.5">
                    <span
                      className="inline-flex items-center gap-1 text-sm font-medium px-2.5 py-1 rounded-full border text-[#0f3d2e] border-[#0f3d2e]/20 bg-[#0f3d2e]/5"
                      aria-label={`Rating: ${PROPERTY_RATINGS[property.id]} out of 5`}
                    >
                      <svg
                      ...
                      {PROPERTY_RATINGS[property.id].toFixed(1)}
                    </span>
                  </div>
            ...
            {reviews.map((review, index) => (
              ...
              >
                <CardHeader className="space-y-6 md:space-y-8 pb-6 md:pb-8">
                  <Quote
                    className="h-10 w-10 md:h-12 md:w-12 text-primary/20 mb-2 md:mb-4"
                    aria-hidden="true"
                  />
